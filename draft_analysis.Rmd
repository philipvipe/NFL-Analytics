...
Load the NFL draft history data, containing the draft data from rounds 1-4 from the 1970-2016 NFL draft. Remove all of the missing data entries, and convert the necessary columns to integers.
```{r cars}
head(master_df)
...
master_df$roty <- as.factor(master_df$roty)
```
...
outcome_name <- 'roty'
predictor_names <- c('Tm', 'Pos', 'Age', 'Pick')
...
model <- train(master_df[,predictor_names], master_df[,outcome_name], 
               method = "gbm",
...
The summaries tell us how much influence each variable had on the model. It was interesting to see the little influence age had in predicting the model. It was also interesting that position had a bigger influence than the team and age.

Now let's use the model to predict the probabilities that each player from the 2020 draft will become Offensive Rookie of the Year. 
```{r eval}

predictions <- predict(object=model, df[,predictor_names], type='prob')
head(predictions)
```